...
if(run.cv){
  ...
  for(k in 1:length(shrinkage_values)){
    cat("k=", k, "\n")
    err_cv[,k] <- cv.function(df.train, shrinkage_values[k], K)
  }
  err_cv$nb.trees <- seq_len(nb_trees)
  err_cv <- tidyr::gather(err_cv, shrinkage, error, X1:X3, factor_key=FALSE)
...
if(run.cv){
  ...
  g <- ggplot(err_cv, aes(nb.trees, error, colour=shrinkage)) +
  geom_line() +
  ylab("CV Error Rate") +
